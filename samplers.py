...
def get_score_from_velocity(vt, xt, t, path_type="linear", reverse=False):
...
def get_tsteps(
    num_steps: int, t_start: float = 1.0, t_end: float = 0.0, shift: float = 1.0, reverse: bool = False
):
...
def euler_sampler(
    ...
    **kwargs
    ...
    t_steps = get_tsteps(num_steps=num_steps, shift=shift, reverse=reverse)
...
def fm_sde_sampler(
    ...
    **kwargs
    ...
    if cfg_scale > 1.0:
        y_null = torch.tensor([1000] * y.size(0), device=_device)
    t_steps = get_tsteps(num_steps=num_steps - 1, t_start=1.0, t_end=1e-4, shift=shift, reverse=reverse)
    ...
    for i, (t_cur, t_nxt) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        s_cur = get_score_from_velocity(v_cur, model_input, t_input, path_type=path_type, reverse=reverse)
...
def adapter_euler_sampler(
    ...
    **kwargs
    ...
    reverse = kwargs.get("reverse", False)
    if cfg_scale > 1.0:
        y_null = torch.tensor([1000] * labels.size(0), device=_device)
    t_steps = get_tsteps(num_steps=num_steps, shift=shift, reverse=reverse)
    # Draw the observation-conditioned initial noise z ~ q_phi(z|y) from the adapter.
    mu, logvar, latents = adapter_model(measurements.to(device=_device, dtype=_dtype), inv_classes, return_sample=True)
    x_nxt = latents.to(torch.float64)
    for i, (t_cur, t_nxt) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        x_cur, y_cur = x_nxt, labels
        model_input = x_cur
        if cfg_scale > 1.0 and t_cur <= g_max and t_cur >= g_min:
            model_input = torch.cat([model_input] * 2, dim=0)
            y_cur = torch.cat([y_cur, y_null], dim=0)
